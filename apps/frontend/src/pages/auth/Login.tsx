...
export default function Login() {
    ...
    const handleRequestOtp = async (e: React.FormEvent) => {
        ...
        try {
            if (isEmail) {
                await requestEmailOtp(identifier, true);
            } else {
                await requestPhoneOtp(identifier, true);
            }
            setOtpSent(true);
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            let data;
            if (method === "email") {
                data = await loginWithEmailOtp(identifier, otp);
            } else {
                data = await loginWithPhoneOtp(identifier, otp);
            }

            if (data?.user?.onboardingStatus === "completed") {
                if (data.user.role === "admin") navigate("/admin/overview");
                else if (data.user.role === "turf_owner")
        ...
        <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
            ...
            <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
                <div className="bg-white py-10 px-4 shadow-2xl sm:rounded-3xl sm:px-10 border border-gray-100">
                    <form
                        className="space-y-6"
                        onSubmit={otpSent ? handleLogin : handleRequestOtp}
                    >
                        {error && (
